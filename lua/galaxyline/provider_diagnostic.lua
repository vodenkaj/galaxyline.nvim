...
local function get_nvim_lsp_diagnostic(diag_type)
  ...
  local count = #vim.diagnostic.get(api.nvim_get_current_buf(), { severity = vim.diagnostic.severity[diag_type]})
...
function M.get_diagnostic_warn()
  ...
  elseif not vim.tbl_isempty(lsp.buf_get_clients(0)) then
    return get_nvim_lsp_diagnostic('WARN')
  end
  return ''
end
...
function M.get_diagnostic_info()
  ...
  elseif not vim.tbl_isempty(lsp.buf_get_clients(0)) then
    return get_nvim_lsp_diagnostic('INFO')
  end
  return ''
end
